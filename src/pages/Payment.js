...
const Payment = () => {
  ...
  const paymentHandler = async (e) => {
    try {
      const res = await axios.post("http://localhost:4500/api/makepayment", {amount: amount * 100, currency,receipt});
      var options = {
        ...
        "handler": async function (response) {
          ...
          if (validateResponse.data.success) {
            const orderResponse = await axios.post("http://localhost:4500/api/create-order", {
              amount,
              address: address,
              userId: user._id,
              cartItems: cart,
